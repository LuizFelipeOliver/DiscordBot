...
class music(commands.Cog):
    YDL_OPTIONS = {'format': 'bestaudio/best', 'noplaylist': 'True', 'nocheckcertificate': True}
    ...
    async def q(self, interaction:discord.Interaction):
        await interaction.response.defer(thinking=True)
        retval = '\n'.join([f'**{i+1} - **{item[0]["title"]}' for i, item in enumerate(self.music_queue)])
